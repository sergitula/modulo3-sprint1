...
export function agregarSuperheroes(rutaOriginal, rutaNuevos) {
    ...
    // AGREGADO PROPIO - Si quiero filtrar por id para evitar duplicados
    // cada vez que se ejecuta el comando "node utils.mjs"
    const idNoExistentes = instanciasNuevos.filter(nuevoHeroe => {
        // some() verifica si al menos un elemento cumple la condición
        return !superheroesOriginales.some(hero => hero.id === nuevoHeroe.id);
    });

    // Combinar listas
    const listaActualizada = [...superheroesOriginales, ...idNoExistentes];
    // const listaActualizada = [...superheroesOriginales, ...instanciasNuevos];
    const contenidoNuevo = JSON.stringify(listaActualizada,null,2);

    // Guardar la lista actualizada
    fs.writeFileSync(rutaOriginal, contenidoNuevo, 'utf8');
